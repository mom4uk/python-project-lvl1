...
def get_progression(initial_value, progression_difference, length_progression):
	maximum_value = (progression_difference * length_progression) + initial_value
	progression = range(initial_value, maximum_value, progression_difference)
	print(progression)
	secret = choice(progression)
	progression_with_secret = ' '.join(['..' if value == secret else str(value) for value in progression])
...
def progression():
	...
	(question, correct_answer) = get_progression(progression_member, progression_difference, LENGTH_ARITHMETIC_PROGRESSION)
